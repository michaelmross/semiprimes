...
Cheap first pass:
    ...
    python3 property_resolve.py d2_challenges_251_999.txt \
        --seconds 3600 --tiers T0,T1

Continue later (journal makes runs resumable):

    python3 property_resolve.py d2_challenges_251_999.txt \
        --seconds 7200 --tiers T2
...
def reconstruct_state(
...
) -> Tuple[Dict[int, Set[int]], Dict[int, Optional[int]], Set[Tuple[int, int, str]]]:
    """
    Return:
    ...
    for r in journal_rows:
        ...
        # INCOMPLETE deliberately does not mark the tier complete.
    ...
    return dead, semiprime_hit, tier_done
...
def make_round_robin_schedule(
    ...
    for n, g in groups.items():
        ...
        xs = [
            m for m in g.candidates
            if m not in dead[n] and (n, m, tier) not in tier_done
        ]
        if xs:
            pending[n] = xs
...
def main() -> None:
    ...
    dead, semiprime_hit, tier_done = reconstruct_state(groups, journal_rows)
    ...
    for tier in tiers:
        ...
        schedule = make_round_robin_schedule(
            groups, dead, semiprime_hit, tier_done, tier
        )
        print(
